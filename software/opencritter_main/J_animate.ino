    ...
    case a_idle:
      ...
      else
      {
        ...
        if (aniStage % 2) //If odd
        {
          ...
          display.drawBitmap(Xpos, Ypos, getFrame(frame_main), 32, 32, 1);
        }
        else
        {
          display.fillRect(Xpos, Ypos, 32, 32, 0);
          display.drawBitmap(Xpos, Ypos, getFrame(frame_idle), 32, 32, 1);
        }
...
const unsigned char* getFrame(byte framename) {
  switch (framename) {
    case frame_main:
      switch (breed) {
        case egg:
...
//THIS IS THE OLD WAY OF ANIMATING STUFF!
